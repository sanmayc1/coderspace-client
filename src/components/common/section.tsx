...
const Section: React.FC<ISectionProps> = ({ reverse = false, bannerUrl, description, head }) => {
  return (
    <>
      <MotionScrollWrap>
        <section className={`grid sm:grid-cols-2 grid-cols-1 px-20 ${reverse ? 'md:gap-32' : ''}`}>
          <div
            className={`${reverse ? 'md:order-2' : 'order-1'} flex flex-col gap-9 justify-center`}
          >
          ...
          <div className={`${reverse ? 'md:order-1' : 'order-2'} flex justify-center items-center`}>
            <img src={`${bannerUrl}`} alt="banner" className="max-w-[85%] min-w-99 " />
          </div>
        </section>
      </MotionScrollWrap>
    </>
