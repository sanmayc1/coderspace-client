...
export async function autoGenerateTestcases(problemId: string): Promise<ICommonResponse> {
  const res = await coderspaceBackend.post(API_ROUTES.AUTO_GENERATE_TESTCASES, { problemId });
  return res.data;
}
