...
export const CompanyRegistreSchema = z
  ...
  export const createContestSchema = z.object({
    ...
    dateAndTime: z.string().min(1, "Date and time is required").refine(
      (v) => !isNaN(Date.parse(v)) && new Date(v) >= new Date(new Date().toISOString().slice(0, 16).replace("T", " ") + ":00"),
      "Date and time must be in the future"
    ),
    duration: z.string().refine((v) => !isNaN(Number(v)) && Number(v) > 0, {
      message: "Duration must be a positive number",
    }),
    visibility: z.string().refine(
      (v) => v === "public" || v === "private",
      "Visibility required"
    ),
    domain: z.string().min(1, "Domain required"),
    // premium optional
    skills: z.array(z.object({ id: z.string(), title: z.string() })).min(1, "At least one skill required"),
    problems: z.array(z.object({ id: z.string(), title: z.string() })).min(1, "At least one problem is required"),
    rewards: z
      .array(
        z.object({
